...
def compute_model_shap_lgbm(
    model, X_test, dataset_name, generator_type, seed, 
    results_dir="results/shap/", figures_dir="figures/shap/"
):
    """
    Computes test set SHAP attributions using TreeExplainer for a synthetic-trained LightGBM model,
    saves the raw values and rankings, and generates Beeswarm and Bar plots.
    """
    print(f"Computing SHAP values for {dataset_name} LightGBM ({generator_type.upper()}, Seed {seed})...")
    
    # Compute SHAP values using TreeExplainer
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(X_test)
    
    # Handle SHAP multi-class format discrepancies
    ...
    shap_val_path = os.path.join(results_dir, f"{dataset_name}_{generator_type}_lgbm_seed{seed}_v1_shap_values.npy")
    ...
    ranking_path = os.path.join(results_dir, f"{dataset_name}_{generator_type}_lgbm_seed{seed}_v1_shap_ranking.csv")
    ...
    rank_pos_path = os.path.join(results_dir, f"{dataset_name}_{generator_type}_lgbm_seed{seed}_v1_shap_rank_position.csv")
    ...
    beeswarm_path = os.path.join(figures_dir, f"{dataset_name}_{generator_type}_lgbm_seed{seed}_v1_shap_beeswarm.png")
    ...
    bar_path = os.path.join(figures_dir, f"{dataset_name}_{generator_type}_lgbm_seed{seed}_v1_shap_bar.png")
    plt.savefig(bar_path, bbox_inches='tight')
    plt.close()
    
    print(f"SHAP calculations and plots completed for {dataset_name} LightGBM ({generator_type.upper()}, Seed {seed})")
    return shap_values_class1

def evaluate_ranking_similarity_lgbm(dataset_name, generator_type, seed, results_dir="results/shap/"):
    """
    Loads real baseline and synthetic rank position files for LightGBM, sorts them,
    and calculates Spearman rank correlation (rho, p-value) and top-K feature overlaps.
    """
    real_path = os.path.join(results_dir, f"{dataset_name}_real_lgbm_seed{seed}_v1_shap_rank_position.csv")
    syn_path = os.path.join(results_dir, f"{dataset_name}_{generator_type}_lgbm_seed{seed}_v1_shap_rank_position.csv")
    
    if not os.path.exists(real_path) or not os.path.exists(syn_path):
        raise FileNotFoundError(f"SHAP rank position files not found for LightGBM. Real: {real_path}, Syn: {syn_path}")
